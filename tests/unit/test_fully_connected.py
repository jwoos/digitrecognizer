...
class TestFullyConnectedInitialization(TestCase):
    ...
    def test_initialization(self):
        input_shape = (1, 100)
        output_shape = (1, 10)
        fc = layers.fully_connected.FC(output_shape[1])
        fc.initialize(input_shape)

        self.assertEqual(fc.input_shape, input_shape)
        self.assertEqual(fc.output_shape, output_shape)
        self.assertTrue(np.array_equal(fc.biases, np.zeros(output_shape[1])))

    def test_initialization_with_bias(self):
        input_shape = (1, 100)
        output_shape = (1, 10)
        fc = layers.fully_connected.FC(output_shape[1], use_biases=True)
        fc.initialize(input_shape)

        self.assertEqual(fc.input_shape, input_shape)
        self.assertEqual(fc.output_shape, output_shape)
        self.assertFalse(np.array_equal(fc.biases, np.zeros(output_shape[1])))
